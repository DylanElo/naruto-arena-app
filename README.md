...
- During the build step it sets `VITE_BASE_PATH` to `/${repo-name}/` (for example `/naruto-arena-app/`) so assets resolve correctly when hosted at `https://<username>.github.io/<repo-name>/`.
...
3. Trigger the "Deploy to GitHub Pages" workflow (push to `main` or use **Run workflow**). Optional: provide a custom base path input if your Pages site needs a different subfolder.
