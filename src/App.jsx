...
function App() {
  ...
  const fullTeamAnalysis = useMemo(() => {
    // Optimization: Skip expensive analysis when not in Builder view
    if (activeTab !== 'builder') return {}
    return analyzeTeam(selectedTeam)
  }, [selectedTeam, activeTab])

  const suggestions = useMemo(() => {
    // Optimization: Skip expensive suggestions when not in Builder view
    if (activeTab !== 'builder') return []
  ...
  const filteredCharacters = useMemo(() => {
    // Optimization: Skip expensive filtering when not in Builder view
    if (activeTab !== 'builder') return []
