...
function App() {
  ...
  // --- ANALYSIS ---
  // Bolt Optimization: Gate analysis when not in builder
  const fullTeamAnalysis = useMemo(() => {
    if (activeTab !== 'builder') return { tempo: {}, missingCapabilities: [] }
  ...
  const filteredCharacters = useMemo(() => {
    // Bolt Optimization: Gate filtering when not in builder
    if (activeTab !== 'builder') return []

    // Bolt Optimization: Hoist search invariant
