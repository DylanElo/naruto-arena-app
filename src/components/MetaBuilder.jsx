...
const MetaBuilder = ({ allCharacters }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-konoha-800 rounded-2xl border border-konoha-700 p-6">
                <h2 className="text-2xl font-bold text-chakra-blue mb-4">📊 Meta Team Generator</h2>
                <p className="text-gray-400 mb-4">Find optimal team compositions from your collection</p>

                {/* Filters */}
                <div className="flex flex-wrap gap-4 mb-4">
                    <div className="flex items-center gap-2">
                        <label htmlFor="max-avg-cost" className="text-sm text-gray-400">Max Avg Cost:</label>
                        <input
                            id="max-avg-cost"
                            type="number"
                            min="1"
                            ...
                            className="w-20 p-1 bg-konoha-900 border border-konoha-700 rounded text-white focus:border-chakra-blue outline-none"
                        />
                    </div>
                    <button
                        onClick={generateTeams}
                        disabled={loading || ownedIds.length < 3}
                        className="bg-chakra-blue hover:bg-white disabled:bg-konoha-700 disabled:text-gray-500 text-black px-6 py-2 rounded font-bold transition-colors clip-tech"
                    >
            ...
            {metaTeams.length > 0 && (
                <div className="space-y-4">
                    {metaTeams.map((teamData, idx) => {
                        ...
                        return (
                            <div key={idx} className="bg-konoha-800 rounded-2xl border border-konoha-700 p-5 hover:border-chakra-blue/50 transition-colors">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="text-xl font-bold text-chakra-blue">
                                            #{idx + 1} Team
                                        </h3>
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-4">
                                    {team.map(char => (
                                        <div key={char.id} className="bg-konoha-900 rounded-lg border border-konoha-700 overflow-hidden">
                                            <div className="flex items-center">
                                                <img
                                                    src={`/naruto-arena-app/images/characters/${char.id}.png`}
                                                    alt={char.name}
                                                    onError={(e) => e.target.style.display = 'none'}
                                                    className="w-20 h-20 object-cover bg-konoha-900"
                                                />
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                    <div className="bg-konoha-900/50 p-2 rounded">
